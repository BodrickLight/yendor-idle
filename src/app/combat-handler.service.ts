...
export class CombatHandlerService {
  ...
  resolveCombat(): void {
    ...
    if (this.rollToHit(this.hero.accuracy, monster.evasion)) {
      this.logger.log(`You hit the ${monster.definition.name}.`, LogType.HeroAttackHit);
      monster.hp.current--;
      if (monster.hp.current <= 0) {
        this.logger.log(`You kill the ${monster.definition.name}!`, LogType.MonsterDeath);
        return;
      }
    } else {
      this.logger.log(`You miss the ${monster.definition.name}.`, LogType.HeroAttackMiss);
    }

    if (this.rollToHit(monster.accuracy, this.hero.evasion)) {
      this.logger.log(`The ${monster.definition.name} hits you!`, LogType.MonsterAttackHit);
      this.hero.hp.current--;
    } else {
      this.logger.log(`The ${monster.definition.name} misses you.`, LogType.MonsterAttackMiss);
    }
  }
